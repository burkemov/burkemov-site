...
- **Cores:** paleta quente sobre base fria em `:root` (`css/styles.css`). `--accent` (#f04e2a) é o único acento dos controles; `--accent-soft` e `--accent-deep` só compõem os gradientes do fundo. Trocar a paleta = trocar esses três mais o `ACCENT` de `tools/gerar-og-image.py` e o `fill` do `assets/favicon.svg`.
- **OG image:** se mudar métricas/slogan, regenere com `tools/gerar-og-image.py` (instruções no topo do script).
> ⚠️ **Trocou uma capa? Suba o `ASSET_VERSION` em `js/main.js`.** As capas mantêm o mesmo nome de arquivo, então o navegador de quem já visitou continuaria mostrando a versão antiga. O sufixo `?v=N` muda a URL e força o download. O `vercel.json` também deixou de marcar `/assets/` como `immutable` (era cache de 1 ano sem revalidar, que foi justamente o que travou capas antigas na tela).
